...
schedule; an in-page ratings editor (worth it around the fourth category);
`storeNearby` gives nearby stores' counts at no extra request cost, so "who else
has it" is nearly free whenever it's wanted.
